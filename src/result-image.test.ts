...
describe("deliverResultPng", () => {
  ...
  it("opens a missing-clipboard post before the png promise settles, and skips a spent download", async () => {
    const order: string[] = [];
    let resolveBlob: (blob: Blob) => void = () => {};
    const png = new Promise<Blob>((resolve) => {
      resolveBlob = resolve;
  ...
  it("does not claim a save when the gesture is already spent", async () => {
