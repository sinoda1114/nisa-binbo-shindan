...
export const DIRECT_SHARE_HINT =
  "画像付きで、XやLINEなどを選べます。要らなければ外せます。";

export type ResultShareOutcome = "shared" | "aborted" | "unavailable";

export type ResultShareTarget = {
  canShare?: (data: ShareData) => boolean;
  share?: (data: ShareData) => Promise<void>;
};

export function resultImageFile(png: Blob, filename: string): File {
  const typed = png.type === "image/png" ? png : new Blob([png], { type: "image/png" });
  return new File([typed], filename, { type: "image/png" });
}
...
export function prepareResultShare(
  ...
  if (typeof target.canShare !== "function") {
    return null;
  }
  const candidates: ShareData[] = [];
  if (title.length > 0 && text.length > 0) {
    candidates.push({ files: [file], text, title });
  }
  if (text.length > 0) {
    candidates.push({ files: [file], text });
  }
  candidates.push({ files: [file] });
  for (const data of candidates) {
    try {
      if (target.canShare(data)) {
        return data;
      }
    } catch {
      continue;
...
export function sharePreparedResult(
  data: ShareData,
  target: ResultShareTarget,
  ...
  try {
    return target.share(data).then(
      () => "shared",
      (error: unknown) => (isShareAbort(error) ? "aborted" : "unavailable"),
    );
  } catch {
    return Promise.resolve("unavailable");
  }
...
export function directShareNotice(outcome: ResultShareOutcome): string {
  switch (outcome) {
    case "shared":
      return "画像付きで共有画面を開きました。";
    case "aborted":
      return "";
    case "unavailable":
      return "このブラウザでは、画像付きのまま共有できません。";
    default: {
      const unreachable: never = outcome;
      return unreachable;
    }
...
export function deliverResultPng(options: {
  ...
  if (!options.clipboard) {
    return options.png instanceof Blob ? Promise.resolve(saveBlob(options.png, true)) : save(true);
  }
